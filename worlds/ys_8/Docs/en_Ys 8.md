...
The Ys 8 AP Randomizer randomizes rewards in the game and adds several items which are used to as world progression 
(all of Dana's past episodes are converted into items that check all events that could be completed from those episodes).

For example, you need to find the `Maiden's Journal` item to spawn the prayer tree to Primordial Passage.

The goal is to defeat the boss in the Selection Sphere, what spawns the boss is the `goal` setting and which boss you fight is an option.

## What items and locations get shuffled?

### Items

Any party member, castaway, item, and a number of event flags that have been converted to items.
...
Please reach out to junglechief87 on Discord or update the issues page on github.
...
- There are now items that upgrade the shops, which includes all shops (Trade, Craft, Tailor, Smith, Brew) and unlocks new weapons to "Reforge"(Access to the shops still requires the requisite castaways that run them first). The items used for this are 7 *Progressive Shop Ranks* and they unlock the upgrades progressively(in a linear order). Late joining characters now have new weapons that are given to them for earlier ranks of shop level when talking to Kathleen, this is to keep them in line with the party in terms of strength. Shop rank 3, The Orichalcum in the original game, has an additional function talking to Kathleen triggers the event where the current party got their weapons upgraded to damage Saurians: Adol, Laxia, Ricotta, Sahad, and Hummel* (Hummel gets an upgrade in the rando from this event, he didn't in the original game). This event also flips the game flag to allow the player to defeat enemies flagged as Saurians, which there are very few enemies out in the world but a lot in intercepts. The first Progressive Shop Rank
gives Kathleen.

- *Progressive Raid Lists* now unlock Intercepts in 5 different stages, progressively(in a linear order). The first unlocks the raid practice, dogi, and options for the first 4 stages.

- The Pirate's *Ship's Log 1* from the East Coast Cave Docks lowers the gangway that grants access to the Eleftheria now, in the original game this event was tied to the treasure chest itself, not finding the logbook.
